...
print("{}: Processing Start.".format(str(datetime.now())))
train = pd.read_csv("/data/Kaggle_Quora/train.csv")
test = pd.read_csv("/data/Kaggle_Quora/test.csv")
...
def process_questions(question_list, questions, question_list_name, dataframe):
    ...
    for question in questions:
        ...
        if len(question_list) % 500000 == 0:
            ...
            print("{}: Question Processing: {} is {}% complete.".format(str(datetime.now()),question_list_name, round(progress, 1)))
...
print("{}: Fitting is complete.".format(str(datetime.now())))
train_question1_word_sequences = tokenizer.texts_to_sequences(train_question1)
print("{}: Tokenization: train_question1 is complete.".format(str(datetime.now())))
train_question2_word_sequences = tokenizer.texts_to_sequences(train_question2)
print("{}: Tokenization: train_question2 is complete".format(str(datetime.now())))

test_question1_word_sequences = tokenizer.texts_to_sequences(test_question1)
print("{}: Tokenization: test_question1 is complete.".format(str(datetime.now())))
test_question2_word_sequences = tokenizer.texts_to_sequences(test_question2)
print("{}: Tokenization: test_question2 is complete.".format(str(datetime.now())))

word_index = tokenizer.word_index
print("{}: Words in index: {}".format(str(datetime.now()), len(word_index)))
...
print("{}: Padding: train_q1 is complete.")

train_q2 = pad_sequences(train_question2_word_sequences, maxlen = max_question_len)
print("{}: Padding: train_q2 is complete.".format(str(datetime.now())))

test_q1 = pad_sequences(test_question1_word_sequences, 
                             maxlen = max_question_len,
                             padding = 'post',
                             truncating = 'post')
print("{}: Padding: test_q1 is complete.".format(str(datetime.now())))

test_q2 = pad_sequences(test_question2_word_sequences, 
                             maxlen = max_question_len,
                             padding = 'post',
                             truncating = 'post')
print("{}: Padding: test_q2 is complete.".format(str(datetime.now())))

y_train = train.is_duplicate

embeddings_index = {}
with open('/data/Kaggle_Quora/glove.6B.300d.txt', encoding='utf-8') as f:
    for line in f:
        values = line.split(' ')
        word = values[0]
...
print("{}: Embedding completed.".format(str(datetime.now())))
...
t0 = time.time()
print("{}: Start training.".format(str(datetime.now())))
...
history = model.fit([train_q1, train_q2, train_q1, train_q2],
                    ...
                    callbacks=callbacks)
t1 = time.time()
print("{}: training completed in {} minutes.".format(str(datetime.now()), ((t1 - t0) / 60.)))
